...
class ProgramCounter():
	...
	#global_inst_address = 0	#inst address within the file
	...
	for line in lines:
		...
		if not ':' in line:
			...
			if line:
				...
				pc.instructions[currLabel].append((line, globalAdress))
				globalAdress += 4
				#pc.in
...
def main():
	...
	while pc.inst_num != len(pc.instructions[pc.curr_label]):
		...
		if pc.jump_flag:
			#print pc.curr_label, curr_inst, pc.instructions[pc.curr_label][pc.inst_num][1]
			if prevInstruction:
				#print addressStream[len(addressStream)-1]
				#print prevInstruction
				#print addressStream[len(addressStream)-1][2], prevInstruction[2]-(addressStream[len(addressStream)-1][2])
				if (addressStream[len(addressStream)-1], prevInstruction-(addressStream[len(addressStream)-1])+4) not in blocksInfo:
					blocksInfo[(addressStream[len(addressStream)-1], prevInstruction-(addressStream[len(addressStream)-1])+4)] = None

			addressStream.append(pc.instructions[pc.curr_label][pc.inst_num][1])
			if (pc.curr_label, pc.inst_num) in blocks.basiblocks:
				blocks.basiblocks[((pc.curr_label, pc.inst_num))] += 1
			else:
				blocks.basiblocks[((pc.curr_label, pc.inst_num))] = 1
		...
		#print args
		prevInstruction = pc.instructions[pc.curr_label][pc.inst_num][1]
